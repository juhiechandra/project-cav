const HEADER = "Cryptorenim";

const NAVBAR_DATA = [
  { id: 1, url: "/", label: "Home" },
  { id: 2, url: "#about-us", label: "About us" },
  { id: 3, url: "#howtostart", label: "How to start" },
  { id: 4, url: "#footer", label: "Contact Us" },
];
const BANNER_DATA = {
  HEADING: "One place for Crypto address validation",
  DECRIPTION:
    "To ensure the security and success of your transaction, choose a coin and carefully enter the correct wallet address, and double check the syntax of the wallet address to avoid any potential errors or issues.",
  HINT: "This is a hint text to help user.",
};
const ABOUT_DATA = {
  HEADING: "Our goal and mission",
  DESCRIPTION:
    "Ensuring the security and accuracy of your cryptocurrency transactions,one wallet address verification at a time.",
  OUR_GOALS: [
    "Our company was founded with the goal of providing a reliable and secure platform for verifying cryptocurrency wallet addresses. ",
    "Our team is dedicated to constantly improving and updating our system to ensure the highest level of accuracy and efficiency.",
    "We prioritize the security of our users and take all necessary measures to protect their personal information and transactions.",
  ],
};
const HOWTOSTART_DATA = {
  HEADING:
    "Quick Guide to Verify Cryptocurrency Wallet Addresses on Our Platform",
  DESCRIPTION:
    "Welcome to our cryptocurrency wallet address verification platform! We are glad to have you on board and hope that our service will be helpful to you. Here are a few steps to get you started:",
  STEPS: [
    "Choose the cryptocurrency that you wish to verify the wallet address. ",
    "Enter the wallet address in the designated field.",
    "Click the Check button to initiate the verification process.",
    "If the wallet address is valid, you will see a confirmation message. If it is invalid, an error message will be displayed.",
  ],
  HINT: "We hope that this manual has been helpful and that you will have a smooth experience using our platform. If you have any further questions or concerns, please don't hesitate to reach out to our support team. We are here to help!",
};

const FOOTER_DATA = {
  HEADING: "Join Our Community",
  HINT: "The security of your money is just as important as the amount you have. Protect it at all costs.",
  CONTACT_DETAILS: {
    ADDRESS: "India",
    MOBILE: "+91 8883332255",
    EMAIL: "cryptorenim@gmail.com",
  },
};

// const FOOTER_DATA = {
//   DESCRIPTION:
//     "We are typically focused on result-based maketing in the digital world. Also, we evaluate your brand’s needs and develop a powerful strategy that maximizes profits.",
//   CONTACT_DETAILS: {
//     HEADING: "Contact us",
//     ADDRESS: "La trobe street docklands, Melbourne",
//     MOBILE: "+1 61234567890",
//     EMAIL: "nixalar@gmail.com",
//   },
//   SUBSCRIBE_NEWSLETTER: "Subscribe newsletter",
//   SUBSCRIBE: "Subscribe",
// };

const MOCK_DATA = {
  HEADER,
  NAVBAR_DATA,
  BANNER_DATA,
  HOWTOSTART_DATA,
  ABOUT_DATA,
  FOOTER_DATA,
};
export default MOCK_DATA;
